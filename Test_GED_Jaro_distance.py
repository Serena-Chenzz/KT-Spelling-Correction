    ...
    try:
        with open(dic_path) as dict:
            for dic_word_line in dict:
                ...
                curr_distance = jellyfish.jaro_winkler(dict_word, word, max(len(dict_word), len(word)))
                if max_distance == None:
                    max_distance = curr_distance
                    correct_words.append(dict_word)
                elif max_distance < curr_distance:
                    max_distance = curr_distance
                    correct_words=[]
                    correct_words.append(dict_word)
                elif max_distance == curr_distance:
...
if __name__ == "__main__":
    ...
    #We only calculate the top 100 words
    file_path = "wiki_misspell.txt"
    file_path_result = "wiki_expected_result_3.txt"
